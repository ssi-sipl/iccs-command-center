...
import { createAlarm } from "@/lib/api/alarms";
...
export default function AddAlarmPage() {
    ...
    const fetchAreas = async () => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          toast({
            title: "Error",
            description: response.error || "Failed to load areas",
            variant: "destructive",
          });
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      !formData.alarm_id.trim() ||
      !formData.name.trim() ||
      !formData.area_id
    ) {
      toast({
        title: "Validation Error",
        description: "Alarm ID, Name, and Area are required",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const response = await createAlarm({
        alarmId: formData.alarm_id.trim(),
        name: formData.name.trim(),
        status: formData.status ? "Active" : "Inactive",
        areaId: formData.area_id || undefined,
      });

      if (response.success) {
        toast({
          title: "Success",
          description: `Alarm "${formData.name}" created successfully`,
        });
        router.push("/alarm");
      } else {
        toast({
          title: "Error",
          description: response.error || "Failed to create alarm",
          variant: "destructive",
        });
      }
    } catch (err) {
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="alarm_id"
                    placeholder="Enter unique Alarm ID"
                    value={formData.alarm_id}
                    onChange={(e) => handleChange("alarm_id", e.target.value)}
                    required
                ...
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger className="border-[#444] bg-[#2a2a2a] text-white focus:ring-[#4A9FD4] disabled:opacity-50">
                      <SelectValue
                        placeholder={
