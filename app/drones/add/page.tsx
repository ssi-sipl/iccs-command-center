// app/drones/add/page.tsx
// Updated Add Drone Page with Backend Integration
"use client";
...
export default function AddDronePage() {
  ...
  const [formData, setFormData] = useState({
    droneId: "", // NEW: Required field
    droneOSName: "",
    droneType: "",
    videoLink: "", // NEW: Optional field
    areaId: "", // NEW: Optional field
    gpsFix: "",
    minHDOP: "",
    minSatCount: "",
    maxWindSpeed: "",
  ...
  const validateForm = () => {
    ...
    if (!formData.gpsLost) {
      errors.gpsLost = "Please select GPS Lost action";
      isValid = false;
    }

    if (!formData.telemetryLost) {
      errors.telemetryLost = "Please select Telemetry Lost action";
      isValid = false;
    }

    if (!formData.batteryFailSafe) {
      errors.batteryFailSafe = "Please select Battery Fail Safe action";
      isValid = false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await createDroneOS({
        droneId: formData.droneId.trim(), // NEW: Required
        droneOSName: formData.droneOSName.trim(),
        droneType: formData.droneType.trim(),
        videoLink: formData.videoLink.trim() || undefined, // NEW: Optional
        areaId: formData.areaId || undefined, // NEW: Optional
        gpsFix: formData.gpsFix.trim(),
        ...
        usbAddress: formData.usbAddress.trim(),
      ...
      if (response.success) {
        toast({
          title: "Success",
          description: `Drone "${formData.droneId}" created successfully`,
        });
        router.push("/drones");
      } else {
        toast({
          title: "Error",
          description: response.error || "Failed to create drone",
          variant: "destructive",
        });
      }
    } catch (err) {
      toast({
        title: "Error",
        description: "Failed to create drone. Please try again.",
        variant: "destructive",
      });
    } finally {
                  ...
                  <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-gray-500">
                        Unique identifier for this drone (e.g., DRONE-001)
                      </p>
                    </div>
                    ...
                    {/* Area Assignment - NEW OPTIONAL FIELD */}
                    <div className="space-y-2">
                ...
                <div className="mb-6">
                  ...
                  <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div className="space-y-2">
                      ...
                      {validationErrors.telemetryLost && (
                        <p className="text-xs text-red-500">
                          {validationErrors.telemetryLost}
                        </p>
                      )}
                    ...
                    <div className="space-y-2">
                      ...
                      {validationErrors.batteryFailSafe && (
                        <p className="text-xs text-red-500">
                          {validationErrors.batteryFailSafe}
                        </p>
                      )}
