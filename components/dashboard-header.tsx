...
import { Eye, Map, Radio, Plane, Bell, Key, FileText, MapPin, BookOpen, LogOut, Menu } from "lucide-react"
...
export function DashboardHeader({ activeItem = "MAIN SCREEN" }: DashboardHeaderProps) {
    ...
    <header className="flex h-14 items-center justify-between border-b border-[#333] bg-[#1a1a1a] px-3 md:px-4">
      ...
      <Link href="/" className="flex items-center gap-2 md:gap-3">
        <div className="flex h-8 w-8 items-center justify-center rounded bg-[#8B0000] md:h-10 md:w-10">
          <svg viewBox="0 0 24 24" className="h-5 w-5 text-[#FFD700] md:h-6 md:w-6" fill="currentColor">
        ...
        <span className="text-base font-bold tracking-wide text-white md:text-xl">RUDRARAKSHAK</span>
      </Link>
      ...
      <div className="hidden items-center gap-4 xl:flex">
        <Button variant="ghost" size="sm" className="gap-2 text-xs text-gray-400 hover:bg-[#333] hover:text-white">
      ...
      <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
        <SheetTrigger asChild>
          <Button variant="ghost" size="icon" className="xl:hidden text-white hover:bg-[#333]">
            <Menu className="h-6 w-6" />
          </Button>
        </SheetTrigger>
        <SheetContent side="right" className="w-72 border-[#333] bg-[#1a1a1a] p-0">
          ...
          <nav className="flex flex-col gap-1 p-4">
            ...
            <Button variant="ghost" className="justify-start gap-3 text-gray-400 hover:bg-[#333] hover:text-white">
